...
export default function VideoChatAdmin() {
	...
	const joinMessage = {
		"typeOfMessage": "JOINADMIN",
		"message":
			`{ "adminId": "${user?.id}","roomId": "${streamId}","accessToken": "${user?.accessToken}"}`
	}
	...
	async function endStream() {
		const endStreamUrl = "http://localhost:8000/api/v1/stream/end-stream"
		try {
			await axios.put(endStreamUrl, { streamId: streamId }, {
				headers: {
